<style>
 ...
 .test{font-size: 14px;}
  ...
  #weiyue{background-color:#fff;display:none;z-index:1;position:fixed;width:33%;top:20%;left:40%;height:40%;border-top:3px solid #222;border-radius:20px;border:1px solid #167ac7 !important;}
    ...
    <div class="span12">
      ...
      <div class="portlet box light-grey">
        <div class="portlet-title">
          ...
          <div class="caption"><i class="icon-globe"></i>收房合同列表</div>
          <div class="tools">
          ...
          <div id="sample_1_wrapper" class="dataTables_wrapper form-inline" role="grid">
            <div class="row-fluid" style="height:116px;">
               <?php if(AdminPositionModul::has_modul("101_01")) {?>
                <form action="/admin/purchasecontract/index" method="get" style="margin:30px;" >
                    <div class="dataTables_filter" style="margin-bottom:10px" id="">
                      <input type="hidden" value="<?php echo $search ?>" name="search">
                      <span class="test">ID：<input type="text" value="<?php echo $k_id;?>" name="k_id"></span>
                      <span class="test">品牌：<input type="text" value="<?php echo $k_estates;?>" name="k_estates"></span>
                      <span class="test">系列：<input type="text"  value="<?php echo $k_building;?>" name="k_building"></span>
                      <span class="test">编号：<input type="text" value="<?php echo $k_room_number;?>" name="k_room_number"></span>
                      <span>
                        </button><input type="checkbox" id="highsearch">高级搜索</button>
                      </span>
                      <button id="sample_editable_1_new" class="btn btn-primary" type="submit" style="height:30px;margin-left:-3px;">
                        搜索 <i class="icon-search"></i>
                      </button>
                    </div>
                    <div id="content" style="display:none;">
                      <div class="dataTables_filter" style="margin-bottom:10px;">
                        <span class="test line21">产权人类型：
                          <select name="k_owner_type" id="">
                              <option value="">请选择</option>
                              <option value="1" <?php echo $k_owner_type==1?'selected':''?>>公司</option>
                              <option value="2" <?php echo $k_owner_type==2?'selected':''?>>个人</option>
                          </select>
                        </span>
                        <span class="test">承租人：<input type="text" value="<?php echo $lessee;?>" name="lessee"></span>
                        <span class="test line22">录入人：<input type="text"  value="<?php echo $k_admin;?>" name="k_admin"></span>
                        <span class="test">状态：
                        ...
                        <span class="test">证件：
              						<select name="k_papers_ok" id="">
              						  <option value="">请选择</option>
              						  <option value="1" <?php echo $k_papers_ok==1?'selected':''?>>全</option>
              						  <option value="2" <?php echo $k_papers_ok==2?'selected':''?>>不全</option>
              						</select>
                        </span>
                      </div>
                      <div class="dataTables_filter" style="margin-bottom:10px;">
                        ...
                        <span class="line4">签约日期：<input type="text" id="datepicker3" value="<?php echo $k_signing_date1;?>" name="k_signing_date1"/>&nbsp;至&nbsp;<input type="text" id="datepicker4" value="<?php echo $k_signing_date2;?>" name="k_signing_date2"/></span>
                        <span class="test">审核：
                ...
                <div class="btn-group pull-right">
                  ...
                  <a href="/admin/purchasecontract/create">
            ...
            <table class="table table-striped table-bordered table-hover" id="sample" ><!-- ID sample_1目前没用,js中控制显示效果 -->
              <thead >
                <tr class="yj-title-th">
                  ...
                  <!-- <th class="">录入人</th> -->
                  <th class="">录入日期</th>
                  <!-- <th class="">单价<br>(元/天/㎡)</th> -->
                  <th class="">签约日</th>
                  <th class="">签约人</th>
                  <th class="">状态</th>
                  <th class="">审核</th>
                  <th class="">证件</th>
                  <?php if(AdminPositionModul::has_modul("101_17")) {?>
                  <th class="">用友编号</th>
                  <?php }?>
                  <th >操作</th>
                  <th >合同复印件</th>
              ...
              <tbody>
                ...
                if($list){
                  ...
                  foreach($list as $model){
                    ?>
                    <tr class="odd gradeX">
                    <td style="color:blue;font-weight:bold !important;"><?php  $talk = ContractSecondtalk::model()->findAll("contract_id = '$model->id' and deleted=0");
                    if($talk){
                        if($talk[count($talk)-1]->result==2){
                          echo  $talk?"谈".count($talk):'';
                        }else{
                          echo  '已优化';
                        }                      
                    }
                ...
                foreach ($res as $key => $value) {
                  ...
                  echo $item?$item->house_no.'<br>':"";
                      ...
                      <td class=" "><?php echo $model->owner_type==1?'公司':'个人'; ?></td>     <!-- <td class=" "><?php $item=AdminUser::model()->find("id='$model->creater_id'"); echo $item?$item->nickname:""; ?></td> -->
            ...
            <?php if(AdminPositionModul::has_modul("101_17")) {?>
            <td>
            <input class="yongyou_id" name="yongyou_id" value="<?php echo $model->yongyou_id ?>" style="width:80px;border:0px solid #aaa!important;text-align:center; vertical-align:middle;">
            </td>
            <?php }?>
                      ...
                      <div class="btn-operation">
                        ...
                        <?php if(AdminPositionModul::has_modul("101_11")) {?>
                          <a href="/admin/More/pur/id/<?php echo $model->id;?>"  style="display:block">打印预览</a>
                        <a href="/admin/purchasecontract/detail/id/<?php echo $model->id;?>"  style="display:block">详情</a>
                        <?php }?>

                        <?php if(AdminPositionModul::has_modul("101_03") && $model->reviewed!=1) {?>
                          <a href="/admin/purchasecontract/edit/id/<?php echo $model->id;?>" style="display:block">编辑</a>
                        <?php }?>
                        <!-- 审核后编辑-->
                        ...
                        <?php if(AdminPositionModul::has_modul("101_04") && $model->reviewed!=1) {?>
                        <a class="del_contract" data-toggle="modal" data-target="#about-modal" href=""  address ="/admin/purchasecontract/delete/id/<?php echo $model->id;?>" style='display:block;'>删除</a>
                        <?php }?>

                        <?php if((AdminPositionModul::has_modul("101_06")||AdminPositionModul::has_modul("101_07"))) {?>
                        <a href="/admin/purchasecontract/payable/id/<?php echo $model->id;?>"  style="display:block">应付</a>
                        <?php }?>

                        <?php if ($model->reviewed!=1 && AdminPositionModul::has_modul("101_05")){?>
                        <a href="/admin/purchasecontract/reviewed/id/<?php echo $model->id;?>"  style="display:block">审核</a>
                        <?php }else if($model->reviewed==1){?>
                        已审核
                        <?php }?>

                        <?php if(AdminPositionModul::has_modul("101_03")&&($model->status==0||$model->status==9||$model->status==-1)) {?>
                        ...
                        <?php if(AdminPositionModul::has_modul("102_16")) {?>
                        <a href="/admin/purchasecontract/secondtalk_list/contract_id/<?php echo $model->id ?>"  style="display:block">二次谈判</a>
                        <?php }?>
                     ...
                     <td>
                      ...
                      <?php if(AdminPositionModul::has_modul("101_13")) {?>
                        <a href="/admin/purchasecontract/copyedit/id/<?php echo $model->id;?>" style="display:block">编辑</a>
                      <?php }?>
                      <?php if(AdminPositionModul::has_modul("101_14")) {?>
                        <a href="/admin/purchasecontract/copydetail/id/<?php echo $model->id;?>" style="display:block">查看</a>
                      <?php }?>
                      <?php if(AdminPositionModul::has_modul("101_15")) {?>
                        <a class="del_copy" data-toggle="modal" data-target="#about-modal" href="" address="/admin/purchasecontract/copydelete/id/<?php echo $model->id;?>" style="display:block">删除</a>
                      <?php }?>
                      </ul>
                    </div>
                     </td>
            ...
            <div id="weiyue" class="portlet-body form" id="form_add"  method="post"  class="form-horizontal js-submit">
              ...
              <form action="/admin/purchasecontract/weiyue" id="form_add"  method="post"  class="form-horizontal js-submit">
            ...
            <div id="signer_edit" class="portlet-body form" id="form_add"  method="post"  class="form-horizontal js-submit">
              <div style="height:40px;background:#167AC7;margin-bottom:20px;" class="portlet-title">
                 <div class="caption" style="line-height:40px;font-size:20px;text-indent:30px;">签约人</div>
              </div>
...
<script>
$(function(){
  $("#weiyue").draggable();//模态框的拖拽性
  $("#closemodel").click(function(){
    $("#weiyue").hide();//关闭模态框
  });

  $("#signer_edit").draggable();//模态框的拖拽性
