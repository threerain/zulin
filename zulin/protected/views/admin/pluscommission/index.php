...
<style>
    ...
    #signer_detail_closemodel{font-size:35px;font-weight:bold;position:absolute;top:10px;right:20px;color:#fff;cursor:pointer;}  
            ...
            <div id="sample_1_wrapper" class="dataTables_wrapper form-inline" role="grid">
              <div class="row-fluid">
                ...
                <div class="span6">
                  ...
                  <a id="purchase" href="/admin/pluscommission/plusindex/type/0">收房提成</a>
                  <a id="sale" href="/admin/pluscommission/index/type/1">出车</a>
                  <a id="sale" href="/admin/pluscommission/plusindex/type/1">出车提成</a>
                  <a id="sale" href="/admin/pluscommission/guanyong/">管佣</a>
                  <form action="/admin/pluscommission/index" style="margin:0" >
                      ...
                      <span>
                        <span class="line4">签约日期：<input type="text" id="datepicker3" value="<?php echo $sign_date1?>" name="signing_date1"/>&nbsp;至&nbsp;<input type="text" id="datepicker4" value="<?php echo $sign_date2 ?>" name="signing_date2"/></span>
                        <script>
                ...
                <div class="span6">
                  <form method="post" action="/admin/pluscommission/judge" id="form_create" class="form-horizontal js-submit" enctype="multipart/form-data">
                      <input type="hidden" name="type" value="<?php echo $type?>">
                      <div class="dataTables_filter" id="sample_1_filter">
                        <span id="specialfont">选择文件：</span> <input type="file" value="" name="file_excel">
                      </div>
                      <div class="btn-group pull-right">
                      <?php
                        if (AdminPositionModul::has_modul("401_01")) {
                      ?>
                      <button id='sfd' type="submit" class="btn btn-primary submit js-btnadd">导入Excel表</button>
                        <i class="icon-plus"></i>
                        </button>
                      <?php }?>
                      </div>
              ...
              <table class="table table-striped table-bordered table-hover" id="sample"><!-- ID sample_1目前没用,js中控制显示效果 -->
                <thead>
                  <tr>
                    ...
                    <th class="hidden-480" style="text-align:center">用友编号</th>
                    ...
                    <th class="hidden-480" style="text-align:center">签约时间</th>
                  ...
                  if($list){
                    ...
                    foreach($list as $model){
                      ...
                      <tr class="odd gradeX">
                        ...
                        <td><?php echo $model->yongyou_id; ?></td>
                        <td>
                        <?php 
                        $arr = Property::allinfo($model->contract_id);
                        foreach ($arr as $key => $value) {
                          echo '品牌:'.$value['estate_name'].' 系列:'.$value['building_name'].' 编号:'.$value['house_no'].'<br>';
                        }
                         ?>
                        </td>
                        <td><a href="javascript:;" class="signer"><?php $item=AdminUser::model()->find("id='$model->signer'"); echo $item?$item->nickname:""; ?></a></td>
                        <td><?php $area = CmsPurchaseProperty::model()->findAll("contract_id = '$model->contract_id'");
                        $totalarea = 0;
                        foreach ($area as $key => $value) {
                          ...
                          echo $totalarea;
                         ?></td>
                        <td class="center hidden-480"><?php echo date('Y-m-d', $model->sign_date); ?></td>
                        <td>
                      ...
                      <div class="btn-operation">
                        ...
                        <ul class="dropdown-menu" style="left: -38px;width:140px !important;">
                          <a href="/admin/notice/detail/id/<?php echo $model->id;?>">查看</a>
                          <?php if(AdminPositionModul::has_modul("401_02")){?>
                            <a href="/admin/notice/edit/id/<?php echo $model->id;?>">编辑</a>
                            <?php }?>
                          <?php if(AdminPositionModul::has_modul("401_03")) {?>
                            <a href="" address="/admin/notice/delete/id/<?php echo $model->id;?>" style="display:block" class="delete" data-toggle="modal" data-target="#about-modal">删除</a>
                          <?php }?>
                        </ul>
                      </div>
                        </td>
            ...
            <div id="signer_detail" class="portlet-body form" id="form_add"  method="post"  class="form-horizontal js-submit">
              ...
              <form action="/admin/purchasecontract/signer_detail" id="form_add"  method="post"  class="form-horizontal js-submit">
                  ...
                  <div class="form-actions">
                   ...
                   <div class="control-group" id="signer_detail_closemodel">
                     ×
                  </div>
              </form>
            </div>
...
$(function(){
  ...
  $("#signer_detail_closemodel").click(function(){
